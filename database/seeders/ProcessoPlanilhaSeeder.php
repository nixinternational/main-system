...
class ProcessoPlanilhaSeeder extends Seeder
{
    ...
    public function run()
    {
        // Apagando todos os processos existentes
        $this->command->info("Apagando processos existentes...");
        // Para PostgreSQL, deletar primeiro os registros filhos
        ProcessoProduto::query()->delete();
        Processo::query()->delete();
        $this->command->info("Processos apagados com sucesso!");
        ...
        $this->command->info("Criando processos para {$clientes->count()} clientes...");
