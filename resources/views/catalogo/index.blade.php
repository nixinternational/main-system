@extends('layouts.app')
@section('title', 'Catálogos')

@section('actions')
    <a href="{{ route('catalogo.create') }}" class="btn btn-primary">
        Cadastrar catálogo
    </a>
@endsection


@section('content')
    <div class="card shadow-sm">
        <div class="card-header" style="background: var(--theme-gradient-primary);">
            <h3 class="card-title mb-0 text-white">
                <i class="fas fa-book me-2"></i>Listagem de Catálogos
            </h3>
        </div>
        <div class="card-body">
            <form class="mb-4" id="formSearch" action="{{ route('catalogo.index') }}" method="GET">
                <div class="row mb-3">
                    <div class="col-md-8 col-sm-12 mb-2 mb-md-0">
                        <div class="input-group">
                            <div class="input-group-prepend">
                                <span class="input-group-text bg-light">
                                    <i class="fas fa-search"></i>
                                </span>
                            </div>
                            <input value="{{ $_GET['search'] ?? '' }}" type="text" id="search" name="search" 
                                class="form-control" placeholder="Buscar por cliente..." 
                                aria-label="Buscar" aria-describedby="basic-addon1">
                            <button type="button" onclick="window.location.href='{{ route('catalogo.index') }}'" 
                                class="btn btn-outline-secondary">
                                <i class="fas fa-times me-1"></i>Limpar
                            </button>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-12">
                        <div class="d-flex justify-content-end">
                            {{ $catalogos->appends(['paginacao' => $_GET['paginacao'] ?? 10])->links() }}
                        </div>
                    </div>
                </div>
            </form>

            @if (!$catalogos->isEmpty())
                <div class="table-responsive">
                    <table id="produtosTable" class="table table-striped table-hover mb-0">
                        <thead style="background: var(--theme-gradient-primary);">
                            <tr>
                                <th class="text-white">Cliente</th>
                                <th>{!! sortable('created_at', 'Data de Cadastro') !!}</th>
                                <th class="text-center text-white">Ações</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($catalogos as $catalogo)
                                <tr>
                                    <td>{{ $catalogo->cliente->nome ?? '-' }}</td>
                                    <td>{{ \Carbon\Carbon::parse($catalogo->created_at)->format('d/m/Y H:i') }}</td>
                                    <td>
                                        <div class="d-flex justify-content-center" style="gap: 8px;">
                                            <a href="{{ route('catalogo.edit', $catalogo->id) }}" 
                                                class="btn btn-sm btn-warning" title="Editar">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form method="POST" action="{{ route('catalogo.destroy', $catalogo->id) }}" 
                                                enctype="multipart/form-data" class="d-inline">
                                                @method('DELETE')
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-danger" title="Excluir">
                                                    <i class="fa fa-trash"></i>
                                                </button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">
                    {{ $catalogos->appends(['paginacao' => $_GET['paginacao'] ?? 10])->links() }}
                </div>
            @else
                <x-not-found />
            @endif
        </div>
    </div>




@endsection

@push('scripts')
    <script>
        document.getElementById('search').addEventListener('change', function() {
            document.getElementById('formSearch').submit()
        })
        document.getElementById('paginacao').addEventListener('change', function() {
            document.getElementById('formSearch').submit()
        })
    </script>
@endpush
